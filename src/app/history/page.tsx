  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-[#121317] border border-gray-800 p-3 rounded-xl shadow-xl max-w-[220px]">
        <p className="text-[9px] text-gray-500 font-bold uppercase tracking-wider">{data.dayFullName}</p>
        <p className="text-xs font-bold text-white mt-1 font-serif">
          Vigor Rating: <span className={data.score < 50 ? 'text-rose-400' : 'text-brand-green'}>{data.score}/100</span>
        </p>
        <div className="mt-2.5 pt-2 border-t border-gray-900 text-[10px] text-gray-400 flex flex-col gap-1.5 font-mono">
        ...
        {data.score < 50 && (
          <p className="text-[9px] text-rose-400 mt-2.5 font-semibold flex items-start gap-1">
...
export default function HistoryPage() {
  ...
  const getAlertBadge = (type: string) => {
    switch (type) {
      case 'dry':
        return <span className="px-2 py-0.5 rounded text-[8px] font-bold bg-brand-copper/10 text-brand-copper border border-brand-copper/15 uppercase tracking-wider">Dry alert</span>;
      case 'overwater':
        return <span className="px-2 py-0.5 rounded text-[8px] font-bold bg-blue-550/10 text-blue-400 border border-blue-500/15 uppercase tracking-wider">Over-sat</span>;
      case 'heat':
        return <span className="px-2 py-0.5 rounded text-[8px] font-bold bg-rose-500/10 text-rose-400 border border-rose-500/15 uppercase tracking-wider">Heat anomaly</span>;
      case 'cold':
        return <span className="px-2 py-0.5 rounded text-[8px] font-bold bg-blue-500/10 text-blue-300 border border-blue-500/15 uppercase tracking-wider">Chill event</span>;
      case 'system':
        return <span className="px-2 py-0.5 rounded text-[8px] font-bold bg-yellow-500/10 text-yellow-400 border border-yellow-500/15 uppercase tracking-wider">Sys trace</span>;
      default:
        return <span className="px-2 py-0.5 rounded text-[8px] font-bold bg-brand-green/10 text-brand-green border border-brand-green/15 uppercase tracking-wider">Record</span>;
    }
  };
    ...
    <div className="min-h-screen bg-obsidian text-gray-100 font-sans flex flex-col justify-between p-4 sm:p-6 pb-28 md:pb-12">
      <div className="max-w-3xl mx-auto w-full flex-grow flex flex-col gap-6 py-2">
        ...
        <header className="flex items-center gap-3 border-b border-gray-900 pb-5">
          ...
          <div>
            <span className="font-serif italic text-brand-gold tracking-wider text-[10px] block">
              R.D.N.K. Private Collection
            </span>
            <h1 className="text-xl sm:text-2xl font-extrabold text-white font-serif tracking-tight">7-Day Vigor Registry</h1>
          </div>
        </header>
        ...
        <section className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="p-4 rounded-xl bg-[#121317]/30 border border-gray-900 shadow-md">
            <span className="text-[9px] font-bold text-gray-500 uppercase tracking-wider block">Rolling Avg</span>
            <h3 className="text-xl sm:text-2xl font-bold font-serif text-brand-green mt-1">80/100</h3>
            <span className="text-[8px] text-brand-green/80 font-bold block mt-1 uppercase tracking-wider">Premium baseline</span>
          </div>

          <div className="p-4 rounded-xl bg-[#121317]/30 border border-gray-900 shadow-md">
            <span className="text-[9px] font-bold text-gray-500 uppercase tracking-wider block">Peak Day</span>
            <h3 className="text-xl sm:text-2xl font-bold font-serif text-brand-green mt-1">Tue &mdash; 86</h3>
            <span className="text-[8px] text-brand-green/80 font-bold block mt-1 uppercase tracking-wider">Max cell turgor</span>
          </div>

          <div className="p-4 rounded-xl bg-[#121317]/30 border border-gray-900 shadow-md">
            <span className="text-[9px] font-bold text-gray-500 uppercase tracking-wider block">Max Stress</span>
            <h3 className="text-xl sm:text-2xl font-bold font-serif text-rose-500 mt-1">Fri &mdash; 48</h3>
            <span className="text-[8px] text-rose-400/80 font-bold block mt-1 uppercase tracking-wider">Env Heatwave</span>
          </div>

          <div className="p-4 rounded-xl bg-[#121317]/30 border border-gray-900 shadow-md">
            <span className="text-[9px] font-bold text-gray-500 uppercase tracking-wider block">Stress Events</span>
            <h3 className="text-xl sm:text-2xl font-bold font-serif text-brand-gold mt-1">{totalStressEvents}</h3>
            <span className="text-[8px] text-brand-gold/80 font-bold block mt-1 uppercase tracking-wider">Self-Corrected</span>
          </div>
        </section>
        ...
        <section className="bg-[#121317]/30 border border-gray-900 rounded-2xl p-4 sm:p-5 shadow-lg flex flex-col gap-4">
          <div className="flex justify-between items-center border-b border-gray-900 pb-3">
            ...
            <span className="text-[9px] font-bold text-gray-400 bg-gray-900 border border-gray-800 px-2.5 py-0.5 rounded uppercase">
              7 Days
            </span>
          </div>
        ...
        <section className="bg-[#121317]/30 border border-gray-900 rounded-2xl p-4 sm:p-5 shadow-lg flex flex-col gap-4">
          <div className="flex items-center gap-2 pb-3 border-b border-gray-900">
            ...
            <h3 className="text-xs font-bold text-white font-serif">Chronological Telemetry Ledger & Auditor Signatures</h3>
          </div>
          ...
          <div className="flex flex-col gap-4 max-h-[380px] overflow-y-auto pr-1">
            ...
            {isMounted && state.alertHistory && state.alertHistory.length > 0 && (
              <div className="flex flex-col gap-3 pb-3 border-b border-gray-900">
                <span className="text-[8px] font-bold text-brand-green uppercase tracking-widest block">Live Signal Events (Active Stream)</span>
            ...
            <div className="flex flex-col gap-3">
              <span className="text-[8px] font-bold text-gray-500 uppercase tracking-widest block">Physiological Auditor Ledger</span>
